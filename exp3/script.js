...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', (event) => {
        ...
        // Get elements (supporting both primary and alternate IDs for exp3 & exp1)
        ...
        const regDateInput = document.getElementById('registration_date') || document.getElementById('reg_date') || document.getElementById('testdate');
        ...
        if (nameInput) {
            ...
            if (nameVal === '') {
                showError(nameInput, `error-${nameInput.id}`, 'Patient Name is required.');
                isValid = false;
                if (!firstInvalidField) firstInvalidField = nameInput;
            } else if (!nameRegex.test(nameVal)) {
                showError(nameInput, `error-${nameInput.id}`, 'Patient Name must contain alphabetic characters and spaces only.');
                isValid = false;
                if (!firstInvalidField) firstInvalidField = nameInput;
            }
        }
        ...
        if (addressInput) {
            ...
            if (addressVal === '') {
                showError(addressInput, `error-${addressInput.id}`, 'Address is required.');
                isValid = false;
        ...
        if (mobileInput) {
            ...
            if (mobileVal === '') {
                showError(mobileInput, `error-${mobileInput.id}`, 'Mobile Number is required.');
                isValid = false;
                if (!firstInvalidField) firstInvalidField = mobileInput;
            } else if (!mobileRegex.test(mobileVal)) {
                showError(mobileInput, `error-${mobileInput.id}`, 'Please enter a valid 10-digit Indian mobile number (e.g. 9876543210).');
                isValid = false;
                if (!firstInvalidField) firstInvalidField = mobileInput;
            }
        }
        ...
        if (emailInput) {
            ...
            if (emailVal === '') {
                showError(emailInput, `error-${emailInput.id}`, 'Email Address is required.');
                isValid = false;
                if (!firstInvalidField) firstInvalidField = emailInput;
            } else if (!emailRegex.test(emailVal)) {
                showError(emailInput, `error-${emailInput.id}`, 'Please enter a valid email address (e.g. patient@example.com).');
                isValid = false;
                if (!firstInvalidField) firstInvalidField = emailInput;
            }
        }
        ...
        if (genderSelect) {
            if (genderSelect.value === '') {
                showError(genderSelect, `error-${genderSelect.id}`, 'Please select a Gender.');
                isValid = false;
        ...
        if (bloodGroupSelect) {
            if (bloodGroupSelect.value === '') {
                showError(bloodGroupSelect, `error-${bloodGroupSelect.id}`, 'Please select a Blood Group.');
                isValid = false;
        ...
        if (deptSelect) {
            if (deptSelect.value === '') {
                showError(deptSelect, `error-${deptSelect.id}`, 'Please select a Department / Appointment Type.');
                isValid = false;
        ...
        if (dobInput) {
            ...
            if (!dobVal) {
                showError(dobInput, `error-${dobInput.id}`, 'Date of Birth is required.');
                isValid = false;
                if (!firstInvalidField) firstInvalidField = dobInput;
            } else {
                const dobDate = new Date(dobVal);
                if (dobDate > today) {
                    showError(dobInput, `error-${dobInput.id}`, 'Date of Birth cannot be in the future.');
                    isValid = false;
                    if (!firstInvalidField) firstInvalidField = dobInput;
                }
            }
        ...
        // 9. Registration Date / Test Date Validation
        if (regDateInput) {
            const regDateVal = regDateInput.value;
            if (!regDateVal) {
                showError(regDateInput, `error-${regDateInput.id}`, 'Registration / Test Date is required.');
                isValid = false;
                if (!firstInvalidField) firstInvalidField = regDateInput;
            } else {
                const regDate = new Date(regDateVal);
                if (regDate > today) {
                    showError(regDateInput, `error-${regDateInput.id}`, 'Registration / Test Date cannot be in the future.');
                    isValid = false;
                    if (!firstInvalidField) firstInvalidField = regDateInput;
                }
            }
